...
import poplib
import sys  # client's mailconfig on sys.path

from . import mail_config
from .mail_parser import MailParser  # for headers matching (4E: .)
from .mail_tool import MailTool, SilentMailTool  # trace control supers (4E: .)

# print('user:', mail_config.popusername)  # script dir, pythonpath, changes
...
class MailFetcher(MailTool):
    ...
    def __init__(self, popserver=None, popuser=None, poppswd=None, hastop=True):
        self.popServer = popserver or mail_config.popservername
        self.popUser = popuser or mail_config.popusername
    ...
    def connect(self):
        ...
        if hasattr(mail_config, 'popusesOneLogin') and self.saveserver:
        ...
        timeout = mail_config.popTimeout

        if getattr(mail_config, 'popusesSSL', False):  # set and True?
        ...
        elif getattr(mail_config, 'popusesTLS', False):  # set and True?
        ...
        if hasattr(mail_config, 'popusesOneLogin'):
    ...
    def disconnect(self, server):
        ...
        if not hasattr(mail_config, 'popusesOneLogin'):
    ...
    fetchEncoding = mail_config.fetchEncoding
    ...
    def downloadAllHeaders(self, progress=None, loadfrom=1):
        ...
        if not self.srvrHasTop:  # not all servers support TOP
            ...
            fetchlimit = mail_config.fetchlimit
        ...
        try:
            ...
            for i in range(loadfrom, msgCount + 1):  # empty if low >= high
                ...
                if fetchlimit and (i <= msgCount - fetchlimit):
                    ...
                    print(f"Message before decode {message}")
                    message = self.decodeFullText(message)
                    print(f"Message after decode {message}")
    ...
    def getPasswordFromFile(self):
        ...
        """
        localfile = open(mail_config.poppasswdfile)
